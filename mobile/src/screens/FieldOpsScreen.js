...
import {
  ...
  STAGE_COLORS,
  getAccessibleFields,
  getAvailableAuditMonths,
  getFieldStages as getFieldStagesForCollection,
  isLogFromMonth
} from '../domain/fieldOperations';
...
export default function FieldOpsScreen({ navigation, route }) {
  ...
  const getFieldStages = React.useCallback(fieldId => getFieldStagesForCollection(fields, fieldId), [fields]);
  ...
  const accessibleFields = getAccessibleFields(fields || [], blockFarms || [], session, activeRole);
  ...
  const availableAuditMonths = React.useMemo(() => getAvailableAuditMonths(logs || []), [logs]);
