...
export default function Table({
  ...
  return (
    <div className={`w-full overflow-x-auto rounded-2xl border border-border bg-surface shadow-xs ${className}`}>
...
export function TablePagination({
  ...
  return (
    <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-t border-border/70 bg-bg/40 text-xs">
      <div className="text-hug-muted font-medium">
        {totalItems != null ? (
          <span>
            Showing page <strong className="text-hug-text">{currentPage}</strong> of{' '}
            <strong className="text-hug-text">{totalPages}</strong> ({totalItems} total records)
          </span>
        ) : (
          <span>
