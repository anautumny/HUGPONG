...
function canonicalScalar(value, field) {
  ...
  if (field === 'quantity' && typeof value === 'object') return JSON.stringify({ value: Number(value.value || 0), unit: String(value.unit || '').trim(), inputName: String(value.inputName || '').trim() });
...
function canonicalItem(item = {}, index = 0) {
  ...
  return { key: String(item.lineItemId || item.id || item.description || index).trim().toLowerCase(), description: String(item.description || item.inputName || `Item ${index + 1}`).trim(), quantity: Number.isFinite(quantity) ? quantity : 0, unit: String(item.unit || '').trim(), unitCost: Number.isFinite(unitCost) ? unitCost : 0, total: Number(item.subtotal ?? item.subTotal ?? (quantity * unitCost)) || 0 };
}
function formatMoney(value) { return `₱${Number(value || 0).toLocaleString('en-PH', { maximumFractionDigits: 2 })}`; }
function formatDate(value, withTime = false) {
  if (!value) return 'Not specified';
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return String(value);
  return date.toLocaleString('en-PH', withTime ? { month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' } : { month: 'long', day: 'numeric', year: 'numeric' });
}
...
function itemSummary(item) { return `${item.quantity} ${item.unit || 'unit'} × ${formatMoney(item.unitCost)} = ${formatMoney(item.total)}`; }
function diffItems(before = [], after = []) {
  const beforeMap = new Map((Array.isArray(before) ? before : []).map(canonicalItem).map(item => [item.key, item]));
  const afterMap = new Map((Array.isArray(after) ? after : []).map(canonicalItem).map(item => [item.key, item]));
  const changes = [];
  for (const key of new Set([...beforeMap.keys(), ...afterMap.keys()])) {
    const previous = beforeMap.get(key); const current = afterMap.get(key);
...
export function formatAmendmentChanges(changes = {}) {
  ...
  Object.entries(changes && typeof changes === 'object' ? changes : {}).forEach(([field, change]) => {
    const before = change?.before ?? change?.from; const after = change?.after ?? change?.to;
    if (itemFields.has(field)) { const itemChanges = diffItems(before, after); if (itemChanges.length) rows.push({ kind: 'group', label: FIELD_LABELS[field], items: itemChanges }); return; }
    if (canonicalScalar(before, field) === canonicalScalar(after, field)) return;
...
export function amendmentEditor(amendment = {}, users = []) {
  ...
  return { id, name: user?.name || user?.displayName || amendment.amendedByName || 'Former or unavailable user', role: user?.role || amendment.amendedByRole || 'Role unavailable' };
}
export function amendmentSummary(amendments = [], users = []) {
  const list = Array.isArray(amendments) ? amendments : []; const latest = list[list.length - 1] || null;
